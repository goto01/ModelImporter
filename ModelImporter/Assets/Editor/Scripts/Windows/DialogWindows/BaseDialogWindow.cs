...
namespace Editor.Windows.DialogWindows
{
	...
	public abstract class BaseDialogWindow<T> : EditorWindow where T : BaseDialogWindow<T>
	{
		...
		protected bool _centerInParentWindow = true;
		...
		{
			Vector2 pos;
			if (_centerInParentWindow) pos = new Vector2(_parentRect.x + _parentRect.width / 2f - _size.x / 2f,
					_parentRect.y + _parentRect.height / 2f - _size.y / 2f);
			else pos = position.position;
			position = new Rect(pos.x, pos.y, _size.x, _size.y);
		}
